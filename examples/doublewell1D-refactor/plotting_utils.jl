...
function plot_parameter_space(θ1rng::Vector, θ2rng::Vector, Q::Matrix, P::Matrix, θsamp::Matrix, θysamp::Matrix)
    with_theme(custom_theme) do
        ...
        # QoI distribution
        hm = heatmap!(ax1, θ1rng, θ2rng, Q, colorrange=(0, 40)) # (-20, 1)
        # contours of ρ(θ)
        ...
        scatter!(ax1, θsamp[1,:], θsamp[2,:], color=(:white, 0.75), markersize=10, label="original density")
        # random samples from AS
        scatter!(ax1, θysamp[1,:], θysamp[2,:], color=(:red, 0.75), markersize=10, label="active subspace")
        # color scale

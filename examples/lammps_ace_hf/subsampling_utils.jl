...
function train_potential(ds_train::DataSet, ace::ACE, dpp_batch::Int64)
    ...
    lp = learn!(lb, ds_train[dpp_inds], 1e-8)

    # cond_num = compute_cond_num(lp)
    return lb, lp, dpp_inds
end 
...
function evaluate_fit(ds_train, ds_test, lb)
    ...
    return Dict{String, Vector}(
        ...
        "f_train" => f_train,
        "f_test" => f_test,
        "e_train_pred" => e_train_pred,
        "e_test_pred" => e_test_pred,
        "f_train_pred" => f_train_pred,
        "f_test_pred" => f_test_pred,
    )
end
...
function update_dist_params(λsamp::Vector, πg::Gibbs, qois::Vector{GibbsQoI}, integrator::ISSamples)
    ...
    for j = 1:J
        ...
        function ϕj(xsamp, λ)
            ...
            return hsamp .* log.(g.(xsamp, (λ,)) ./ Zg_quad(λ))
        end

        logwt(x) = logupdf.((qj.p,), x) - logpdf(integrator.g, x, GQint)
        ...
        @time res = optimize(entlog, λsamp[j], NelderMead(),
                 Optim.Options(f_tol=1e-3, g_tol=1e-6, f_calls_limit=100)) # ; autodiff = :forward)
...
function update_mixture_wts(αsamp::Vector, λsamp::Vector, πg::Gibbs, qois::Vector{GibbsQoI}, integrator::ISSamples)
    ...
    logwt(x,α,j) = 2*logupdf.((πfj(j),), x) - logpdf(πgα(α), x, GQint) - logpdf(integrator.g, x, GQint)

    function varj(α,j)
        α = normalize(α)
        M = maximum(logwt(xsamp, α, j))
        return sum( ϕj.(xsamp,(j,))'*ϕj.(xsamp,(j,)) .* exp.(logwt(xsamp, α, j) .- M) ) / sum( exp.(logwt(xsamp, α, j) .- M) )
    end
...
function adapt_mixture_biasing_dist(λsamp::Vector, q::GibbsQoI, πg::Gibbs; N0=5000, Nk=N0, niter=5)

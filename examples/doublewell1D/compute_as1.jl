...
for n = 1:length(nsamp_arr)
    ...
    for j = 1:nrepl
        ...
        xsamp = rand(πu, nMC)
        t = @elapsed begin
            CIS_u[j], metrics_u["wvar"][j], metrics_u["wESS"][j], metrics_u["wdiag"][j] = compute_covmatrix_IS(q, πu, xsamp, θsamp, gradh=∇h)
        end
        ...
        for i = 1:nβ
            ...
            t = @elapsed begin
                CIS_g[βi][j], metrics_g[βi]["wvar"][j], metrics_g[βi]["wESS"][j], metrics_g[βi]["wdiag"][j] = compute_covmatrix_IS(q, πg, xsamp, θsamp, gradh=∇h)
            end
    ...
    JLD.save("data/DW1D_MC_nsamp={$nsamp}_nrepl={$nrepl}.jld",
        "n_θsamp", nsamp,
        "n_xsamp", nMC,
        "C", CMC,
    )
